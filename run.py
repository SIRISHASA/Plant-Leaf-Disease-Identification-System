...
global rdir
rdir="G:\Soft\python_ex\Leaf_Disease_Python\Code"
...
conn = sqlite3.connect('db-sqlite3/Leaf_Disease.db')
with conn:
    cursor=conn.cursor()
#status=StringVar()
#status=""
...
class RootPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label_0 = Label(self, text="leaf disease detection",width=20,font=("bold", 20))
...
class AdmHmPg(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def AdmAdUsr():
            ...
            def database():
                ...
                em1=Email.get()
                nm1=name.get()   
                #cursor.execute('CREATE TABLE IF NOT EXISTS user (`ID` INTEGER PRIMARY KEY AUTOINCREMENT,`disease_name` TEXT,`mobile` NUMERIC,`email` TEXT,`crtype` TEXT,`address` TEXT,`dirurl` TEXT)')
        ...
        def AdmAdCr():
            ...
            def database():
                ...
                row=cursor.execute("SELECT id FROM disease")
                row=cursor.fetchall()
                global rid
                rid=len(row)
                rid=rid+1
                ...
                #cursor.execute('CREATE TABLE IF NOT EXISTS disease (`ID` INTEGER PRIMARY KEY AUTOINCREMENT,`disease_name` TEXT,`mobile` NUMERIC,`email` TEXT,`crtype` TEXT,`address` TEXT,`dirurl` TEXT)')
                cursor.execute('INSERT INTO disease (id,disease_name,email,remark) VALUES(?,?,?,?)',(rid,name1,emaild,crt))
                conn.commit()
                ...
                for i in range(len(crfps)):
                    ...
                    cursor.execute('INSERT INTO dlLoc (crid,fploc) VALUES(?,?)',(rid,fp))
                    conn.commit()

       
       
                 
            label_0 = Label(win, text="Add Disease",width=20,font=("bold", 20))
            label_0.place(x=90,y=53)


            label_1 = Label(win, text="Disease Name",width=20,font=("bold", 10))
            label_1.place(x=80,y=130)
            ...
            Button(win, text='Add Disese',width=20,bg='green',fg='white',command=crpf).place(x=180,y=480)
    ...
    """
        ...
        def AdmVwCr():
            ...
            label_0 = Label(win, text="Disease Added",width=20,font=("bold", 20))
            label_0.place(x=100,y=53)

            Button(win, text='Exit',width=8,bg='black',fg='white',command=win.destroy).place(x=390,y=53)

            row=cursor.execute("SELECT `disease_name`,`email` FROM disease")
            row=cursor.fetchall()
        ...
        label_2 = Label(self, text="Disease Section",width=20,font=("default",14))
        label_2.place(x=100,y=260)
        
        
        Button(self, text='Add Disease',width=24,bg='brown',fg='white',command=AdmAdCr).place(x=123,y=290)
        Button(self, text='View Disease',width=24,bg='brown',fg='white',command=AdmVwCr).place(x=123,y=320)
    ...
    def __init__(self, parent, controller):
        ...
        def Process():
            ...
            def main():
                ...
                #c=len(tags)
                #print c"""
                fpd=cursor.execute("SELECT fploc FROM dlLoc")
                fpd=cursor.fetchall()
                #fpdle=len(fpd)
                global cnt
                cnt=1
                ...
                label_1 = Label(wipro, text="Total Disase in DB : ",width=20,font=("bold", 10))
                label_1.place(x=80,y=130)
                ...
                try:
                    for i in range(len(fpd)):
                        ...
                        score_threshold = 1
                        cnt=cnt+1
                        print("score/len(matches): ",str(score/len(matches)))
                        print("score_threshold: "+str(score_threshold))
                        if score/len(matches) < score_threshold:
                            print("Disease matches.")
                            print ("Match found with following Disease : ")
                            print('Score : {:.2f}'.format(score))
                            def showmatch():
                                win=Toplevel()
                                win.geometry('500x700')
                                win.title('Disease Matched!')

                                getcrid=cursor.execute("SELECT crid from dlLoc where fploc=?",(mfpd,))
                                getcrid=cursor.fetchall()
                                gcrid=getcrid[0][0]
                                #print(gcrid)
                                #exit()
                                gcrdet=cursor.execute("SELECT disease_name,email,remark from disease where id=?",(gcrid,))
                                gcrdet=cursor.fetchall()
                                     
                                label_0 = Label(win, text="Image Matched Disease",width=20,font=("bold", 20))
                                label_0.place(x=90,y=53)
                                ...
                                entry_2 = Label(win,text=""+str(gcrdet[0][1]))
                                entry_2.place(x=240,y=230)
                                ...
                                crt = Label(win,text=""+str(gcrdet[0][2]))
                                crt.place(x=240,y=380)
                        ...
                        """
                            ...
                            win=Toplevel()
                            win.geometry('500x500')
                            win.title('Disase Matched!')"""
                            break
                            #print (img1)
                        else:

                            print("Disase does not match.")
                            print('Score : {:.2f}'.format(score))
                            #print("Score : "+score)
        ...
        def UsrInpFp():
            #print("Hello")
            global fp2
            fp2 = tkinter.filedialog.askopenfilename(initialdir = "DiseaseDataset/",title = "Select file",filetypes = (("TIFF files","*.tif"),("all files","*.*")))
            Process()
        #Main Admin Section
        if (usrnm=="None"):
            label_0 = Label(self, text="Hello, Welcome!!",width=20,font=("bold", 20))
            label_0.place(x=100,y=53)
        else:
        ...
        ttk.Separator(self).place(x=0, y=120, relwidth=1)
        label_1 = Label(self, text="Leaf Image",width=20,font=("default", 14))
        label_1.place(x=100,y=130)
        
        
        Button(self, text='Input Leaf Image',width=24,bg='brown',fg='white',command=UsrInpFp).place(x=123,y=160)
        ...
        def AdmChPwd():
            ...
            def updAdmPwd():
                ...
                print(nwpwd1)
                #cursor.execute('CREATE TABLE IF NOT EXISTS criminal (`ID` INTEGER PRIMARY KEY AUTOINCREMENT,`name` TEXT,`mobile` NUMERIC,`email` TEXT,`crtype` TEXT,`address` TEXT,`dirurl` TEXT)')
                cursor.execute('UPDATE user set pwd=? where unm=?',(nwpwd1,umane_e,))
                conn.commit()
                cp = Label(win, text="Succesfully Changed Password!",fg='Green',width=40,font=("bold", 10))
                cp.place(x=100,y=90)
                cp = Label(win, text="Please Close this Window.",fg='red',width=40,font=("bold", 10))
            ...
            def vrPwd():
                n=1
                cpwd=crpwd.get()
                row=cursor.execute("SELECT `pwd` FROM user where unm=?", (umane_e,))
                row=cursor.fetchall()
                print(row[0][0])
                if(cpwd==row[0][0]):
                    icrpwd.config(state="disable")
            ...
            Button(win, text='Exit',width=8,bg='black',fg='white',command=win.destroy).place(x=390,y=53)
            """
            row=cursor.execute("SELECT `pwd` FROM user where id=1")
            row=cursor.fetchall()
            print(row[0][0])
            """
            label_1 = Label(win, text="Current Password",width=20,font=("bold", 10))
    ...
    def __init__(self, parent, controller):
        ...
        def database():
            ...
            with conn:
                cursor=conn.cursor()
            #cursor.execute('CREATE TABLE IF NOT EXISTS criminal (`ID` INTEGER PRIMARY KEY AUTOINCREMENT,`name` TEXT,`mobile` NUMERIC,`email` TEXT,`crtype` TEXT,`address` TEXT,`dirurl` TEXT)')
            cursor.execute('INSERT INTO disease (name,gender,email,mobile,address,crtype,state) VALUES(?,?,?,?,?,?,?)',(name1,gender,email,mbl,addr,crt,state))
            conn.commit()
